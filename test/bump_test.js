...
exports.bump = {
    ...
    git: function (test) {
        test.expect(1);
        testbump('bump:git', function (result) {
            test.ok(grunt.file.readJSON(tmp).version.search(/\d+(\.\d)+\+sha\.[\w]+/) !== -1, 'Should have bumped git version.');
            test.done();
        });
    },
    noargs: function (test) {
        ...
        grunt.util.spawn({
            ...
            test.equal(String(result),
                    'Running "bump" task\n>> Error: no modes specified.\n>> Valid modes are: major, minor, patch, prerelease, git.' +
